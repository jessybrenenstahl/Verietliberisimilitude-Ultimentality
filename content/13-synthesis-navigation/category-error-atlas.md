---
slug: category-error-atlas
title: The Category-Error Atlas
section: synthesis-and-navigation
page_type: navigation
formal_status:
  epistemic: Derived
  tier: Exposition
  alethic: aspires to map the category errors that actually recur across the wiki; a miscarved or missing row is inaccurate, not merely disallowed
prerequisites: [prohibited-collapses, same-level-attack-rule, two-mark-system]
regulates: ["cross-layer attack (contesting a claim from the wrong altitude)", "type collapse", "primitive/binding inversion"]
regulated_by: [same-level-attack-rule, attack-type-matching, prohibited-collapses]
valid_attack: "A listed row is not a category error at the level claimed, or a genuine recurring cross-layer error has no entry and no near category."
isolation_failure: "Without the same-level rule and the prohibited-collapses index, the atlas degrades into a shield that retypes valid same-level attacks as category errors."
kill_condition: "A same-level counter-instance is repeatedly deflected by citing an atlas row rather than answered, i.e., the atlas is used to retype valid attacks instead of to catch invalid ones."
see_also: [prohibited-collapses, same-level-attack-rule, attack-type-matching, outcomes-of-attack, cardinal-error, interaction-matrix, kill-table]
application_tags: []
---
...
- **cardinal** — the crossing breaks a framework invariant: the [[cardinal-error|cardinal error]] of naming a binding a primitive, or the [[textual-nephilim|Textual Nephilim]] of promoting a carving to a forced tier. A cardinal collapse falsifies the framework where it stands, not merely a page.
...
Each row carries one or more **trigger phrases**. When a trigger phrase occurs on any page, search surfaces the matching row inline and links to its corrective pages — the atlas is the destination that turns an intercepted phrase into a repair, the mirror-image of [[prohibited-collapses|The Prohibited Collapses]]. The `regression test` column is the coupling to [[negative-information|Negative Information]] and the [[scar-record|Scar Record]]: the test is the *present constraint* that keeps a once-repaired error from returning silently. A row whose regression test no longer protects the current structure is inert and is **deleted**, not preserved as decoration — an atlas that only accumulates is a museum, not a regulator. Machine-readable rows feed the same relation surface as the [[interaction-matrix|Interaction Matrix]].
...
- **An atlas hit does not confirm the framework.** Catching an invalid, cross-layer attack removes one objection; it is never evidence that the targeted claim is true. Failed attacks route to [[outcomes-of-attack|The Outcomes of Attack]] and are logged *as failed attacks*, never counted as confirmation.
- **The atlas is not a shield against same-level attacks.** When a counter-instance is offered at the claim's own altitude, no row applies, and citing a row to deflect it is exactly the self-sealing move the [[self-sealing-test|Self-Sealing Test]] and [[causal-falsifiability|Causal Falsifiability]] are built to catch. Every row includes an appeal path back to those pages.
...
- **The categories are not a closed or forced set.** New recurring errors earn new rows; the count is a carving, and treating it as complete or foundational is the [[textual-nephilim|Textual Nephilim]] in miniature.
