...
| Force | coupled controller ⊕ | removal test or failed mutual regulation; show ⊕ = + | additive collapse (⊕ read as sum) | the specified organization of its bindings | read as a sum → the emergent coupling behavior is lost | motive structure absent from either regulator alone | a Force whose behavior is fully recovered by adding its parts | decouple the two regulators; the emergent behavior must vanish | [[force]] |
...
| Reconciliation | coupled controller (Apology ⊕ Gratitude) | removal test; ineffective coupling | apparent repair or stalled self-correction | correction informing outward value | loss of correction or loss of outward participation | renewed participation | the named operation is recoverable without either binding | compare the composite with each component removed; distinguish composition from object | [[reconciliation]] |
...
- **Reconciliation has unrestricted objects.** The row identifies its emergent composition from Apology and Gratitude. No selected object establishes a privileged relationship to the process.
