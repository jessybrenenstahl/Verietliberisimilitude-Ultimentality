---
slug: closure-without-totalization
title: Closure Without Totalization
...
- **The distinction read as a shield for failed formulations.** Distinguishing the condition `C` from a formulation `c_i` does not immunize every `c_i`; a formulation may be deleted, narrowed, or replaced. Using the distinction to rescue any broken statement is the retreat [[frame-condition-and-formulation|frame condition and formulation]] forbids.
- **"Not exhaustive" read as "not closed."** Refusing completeness is not refusing closure. Denying the medium's closure to avoid totalization trades one failure (totalization) for its paired failure (regress).
