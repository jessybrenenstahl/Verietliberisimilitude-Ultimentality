...
isolation_failure: "Stability made immune to correction becomes self-certification; revision without continuity criteria lets unrelated replacements inherit the name."
kill_condition: "Show that the operational tests falsely identify causal correction, or that the criteria cannot distinguish continuity from replacement."
...
The marking operation at time `t`, applied to its own present formulation, yields `M_(t+1)`: a possibly revised marking operation. `M_(t+1)` can equal `M_t` where review warrants retention. Equality of successive formulations does not imply self-certification. In parallel, the closure condition `C` at time `t`, run through the marking operation `M_t`, transitions to `C_(t+1)`. Both the marker and the marked formulation of closure remain revisable. The point that stays fixed is the *mode* — that marking still runs, still couples to correction, still leaves a live kill condition — not the sentence in which the mode is currently written.
...
A fixed-point equation states invariance under an operation; it does not itself assert correctness or immunity. The dynamic account concerns continuity across possible revisions, including justified retention. It is a functional model, not a theorem that every review must change its input.

## Type and formal status

**E:** Derived, CV. The marking discipline requires continued correctability, not continual alteration (see [[self-application|Self-Application]]). But the fixed-point vocabulary, the two-line notation, and the three continuity criteria are a *formalization* — a carving contestable by counter-instance or by a better model of what "the same operation persisting" means. It is a treatise-side extension, held contestable, not a canonical or founded claim. Promoting the fixed point to a settled, unmarked invariant would be exactly the [[textual-nephilim|Textual Nephilim]] the page exists to forbid.
...
- a transition `M_t → M_(t+1)` whose continuity cannot be established remains unverified as a continuation; missing evidence alone does not prove arbitrary mutation. Trace the preserved function and the grounds for the change.
...
- [[causal-error-mark|The Error Mark With Causal Force]] supplies the requirement that relevant error remain capable of changing the governing formulation. A justified extension or improvement need not begin with a refuted prior statement. Without it, "the operation persists" degrades into unregulated drift.
...
- Show that an operational test of all three criteria admits a captured apparatus despite claiming to establish effective correction. Merely satisfying two criteria while failing the third is an excluded case, not a counterexample to their conjunction.
...
The claim dies if either half of its stated kill condition is met. First: show that the proposed tests falsely report causal correction where no relevant error can change the governing formulation. That challenges the tests. A system that actually lacks causal correction fails criterion 2; preserving labels alone does not satisfy the full conjunction. Second: show the criteria **cannot distinguish continuity from replacement** — that any successor, corrective or arbitrary, passes them equally — so "the same apparatus persisting" carries no operational content and the fixed point is empty. Either result forces deletion or reformulation of the carving; a defeated attack on the criteria is logged as a defeated attack, never as proof that the fixed point is permanent.

## Prohibited misreadings

- **`M(M)=M` as permanent self-approval.** The prohibited static reading. The fixed point does not certify the apparatus as settled; it names an operation that stays capable of revising its own formulations, and can retain them when warranted.
